...
void setup() {
  ...
  last_sustain_pedal_value = map(analogRead(PIN_SUSTAIN_PEDAL), 0, 1023, 0, 127);
}
...
void loop() {
  ...
  /* 서스테인 페달 컨트롤러 */
  int sustain_pedal_value = map(analogRead(PIN_SUSTAIN_PEDAL), 0, 1023, 0, 127);
  if (sustain_pedal_value <= 3) {
    sustain_pedal_value = 0;
  }
  
  if (abs(last_sustain_pedal_value - sustain_pedal_value) > 1) { // 값이 달라질때만 신호 보냄
    last_sustain_pedal_value = sustain_pedal_value;
    midi_send(CONTROL_CHANGE, CHANAL, CC_SUSTAIN_PEDAL, sustain_pedal_value);
  }
  /********************/
}
